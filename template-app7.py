...
def init_database():
    ...
    try:
        ...
        # Create streaming_profiles table for reusable streaming configurations
        cursor.execute('''
...
def save_streaming_profile(profile_name, title, description, tags, category, privacy_status, made_for_kids):
    """Save streaming profile for reuse"""
    try:
        conn = sqlite3.connect("streaming_logs.db")
        cursor = conn.cursor()
        
        cursor.execute('''
            INSERT OR REPLACE INTO streaming_profiles 
            (profile_name, title, description, tags, category, privacy_status, made_for_kids, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (
            profile_name,
            title,
        ...
        st.error(f"Error saving streaming profile: {e}")
        return False

def load_streaming_profiles():
    """Load saved streaming profiles"""
    try:
        conn = sqlite3.connect("streaming_logs.db")
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT profile_name, title, description, tags, category, privacy_status, made_for_kids
        ...
        profiles = []
        for row in cursor.fetchall():
            profile_name, title, description, tags, category, privacy_status, made_for_kids = row
            profiles.append({
        ...
        st.error(f"Error loading streaming profiles: {e}")
        return []

def delete_streaming_profile(profile_name):
    """Delete a streaming profile"""
    try:
        conn = sqlite3.connect("streaming_logs.db")
        cursor = conn.cursor()
        
        cursor.execute('DELETE FROM streaming_profiles WHERE profile_name = ?', (profile_name,))
        
        conn.commit()
        conn.close()
        return True
    except Exception as e:
        st.error(f"Error deleting streaming profile: {e}")
        return False
...
def main():
    ...
    with col1:
        ...
        st.subheader("📋 Streaming Profiles")
        with st.expander("💾 Manage Streaming Profiles"):
            # Load existing profiles
            profiles = load_streaming_profiles()
            
            if profiles:
                st.write("**Saved Profiles:**")
                for profile in profiles:
                    col_prof1, col_prof2, col_prof3 = st.columns([3, 1, 1])
                    with col_prof1:
                        st.write(f"📝 {profile['profile_name']}")
                    with col_prof2:
                        if st.button("✅ Use", key=f"use_profile_{profile['profile_name']}"):
                            # Apply profile settings
                            st.session_state['stream_title_input'] = profile['title']
                            st.session_state['stream_description_input'] = profile['description']
                            st.session_state['tags_input'] = ", ".join(profile['tags'])
                            st.session_state['category_id'] = profile['category']
                            st.session_state['privacy_status'] = profile['privacy_status']
                            st.session_state['made_for_kids'] = profile['made_for_kids']
                            st.success(f"✅ Applied profile: {profile['profile_name']}")
                            st.rerun()
                    with col_prof3:
                        if st.button("🗑️ Delete", key=f"delete_profile_{profile['profile_name']}"):
                            delete_streaming_profile(profile['profile_name'])
                            st.success(f"🗑️ Deleted profile: {profile['profile_name']}")
                            st.rerun()
            else:
                st.info("No saved profiles yet. Create one below!")
            
            st.markdown("---")
            st.write("**Create New Profile:**")
            profile_name = st.text_input("Profile Name")
            if st.button("💾 Save Current Settings as Profile") and profile_name:
                # Get current settings
                current_title = st.session_state.get('stream_title_input', 'Live Stream')
                current_description = st.session_state.get('stream_description_input', 'Live streaming session')
                current_tags_input = st.session_state.get('tags_input', '')
                current_tags = [tag.strip() for tag in current_tags_input.split(",") if tag.strip()] if current_tags_input else []
                current_category = st.session_state.get('category_id', "20")
                current_privacy = st.session_state.get('privacy_status', "public")
                current_kids = st.session_state.get('made_for_kids', False)
                
                if save_streaming_profile(
                    profile_name, 
                    current_title, 
                    current_description, 
                    current_tags, 
